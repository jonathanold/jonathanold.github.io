...
df <- data.frame(Person = rep(1:4,50)) %>%
  mutate(X = .5+.5*(5*Person-5) -4 + rnorm(200)) %>%
  mutate(Y = -.9*X + (10*Person-10) -13 + rnorm(200),time="1") %>%
  group_by(Person) %>%
...
animate(p,nframes=200, height = 500, width = 500)

anim_save("/Users/jonathanold/Library/CloudStorage/GoogleDrive-jonathan_old@berkeley.edu/My Drive/_Berkeley Teaching/Econ 140-S2023/fe.gif")
anim_save("/Users/jonathanold/Documents/GitHub/jonathanold.github.io/static/images/fe.gif")
